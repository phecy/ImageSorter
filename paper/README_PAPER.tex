...
Because this is sensitive to subject and movement, we then take the bounding box derived in \ref{ContentRecognition} and rerun the color channel similarity algorithm with stricter threshold of 10\%.

A final similarity index \(S\) is found by adding the scaled timestamp to the results of the color distribution algorithms:
\[
S_{i,j}=\frac{TW_TD_1}{R} + avg(D_1,D_2)
\]
Where \(T\) is the timestamp similarity index, \(W_T\) is the timestamp weight (empirically chosen to be 30\%), \(R\) is the range of possible ranks, \(D_1\) is the color distribution of the entire image, and \(D_2\) is the color distribution of the foreground.

Similar images are clustered with a Quality-Threshold based clustering algorithm.
