...
% Hope I didnt completely kill it. I was removing itemization and just putting in the text I hope. - JED
...
UUU: We assess exposure quality using two measurements global and local. 
The global method determines how far the average value of the image from the "middle grey" value. When the average value of the image is the sum of
each pixel's brightness value, divided by the number of pixels.  
\begin{eqnarray}
Image_i avg =\frac{\sum\limits_{i=1}^nGrey_i}{n}
\end{eqnarray}

The local method uses a nxm grid. The size of the grid is determened by the size of the image. We suggest that each
cell of a grid wil not exceed 2000 pixels. For each cell we calculate average grey values using equation UUU as
in the global method. 

We combine the global and local values we aqcired in order to estimate the exposure value of an image.
...
UUU: should have three paragraphs.
removal, quality assesment and similarity.

for quality assessment - camera is the reference, it's light meter.
looked for studies/papers about light meters in a camera.
found only patents, and they are not what we need.
anyway, the site that you like says: http://www.cambridgeincolour.com/tutorials/camera-metering.htm
...
-> not sure that this reference is good. btw, never met this argument in the papers i read...

similarity - turk is ok. they just asked for more details, how many turkers, how many sets each...

removal - i don't think we have anything right now. we can just hand wave and say that combination of prev two provides some good (subjective?) results


We asked Turk users to group together photographs within four sets of twenty images based on their own personal measure of "similarity." We agree with Turk users' decisions of whether a given pair of images is in the same group with 78\% accuracy. We then grouped together photographs with the same subject in focus and achieved an 85\% accuracy on the same dataset. % JAMES SHOULD THIS LAST MEASURE BE HERE?
