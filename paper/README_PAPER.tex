...
\[
\frac{1}{R}\left(\displaystyle\sum\limits_{i=1}^n\left(W_i\sqrt[3]{Q_i^2-t}\right)\right)^3
\]
Where there are \(n\) modules, \(W_i\) represents the weight of module \(i\), \(Q_i\) is that module's rating for the image, \(R\) is the range of possible ratings, and \(t\) is a chosen threshold, \(0<t<R\). Weights are assigned as follows:
\begin{itemize}
\item \(W_{exposure}=50\%\)
\item \(W_{blur}=40\%\)
\item \(W_{color}=10\%\)
...
\begin{figure*}
\centering
\subfloat[]{\includegraphics[scale=.27]{example_expose.eps}}
\subfloat[]{\includegraphics[scale=.27]{example_blur.eps}}
\subfloat[]{\includegraphics[scale=.27]{example_good.eps}}
\caption{Some example image ratings}
\label{fig:Examples}
\end{figure*}
...
Using the 48 image data set from Ke, Tang, and Jing\cite{1640788}, and Amazon Turk ratings as a ground truth, we achieve a correlation coefficient of \(.266\), which is comparable to the authors' correlation coefficient of \(.495\). Similarly, with a sample from Luo and Tang's public data set\cite{springerlink:10.1007/978-3-540-88690-7_29}, we achieve a correlation coefficient of \(.209\). These numbers show that our algorithm provides positive absolute results when comparing independent, despite our goal of culling images within a set.
