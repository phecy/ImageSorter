...
Digitization of cameras has made photography accessable to a larger audience than ever before.  Photographic collections can easily become larger than any one photographer has time to handle because of the instantaneous transition from in-camera to computer library.  The first culling of photographs needs to eliminate those unworthy of human attention; the technically flawed and otherwise poor-quality images.  To automate this process, an aesthetics ranking technique can be implemented, but this method would discard many images with potential to be fixed through retouching.
...
Previous works \cite{1640788}, \cite{springerlink:10.1007/11744078_23}, \cite{springerlink:10.1007/978-3-540-88690-7_29} assessed image quality to be able to tell whether the image was taken by a professional photographer or by an amatur user, and got very high accuracy in binarry classification of bad and good images. This allows to retrieve only high quality images from a collection of images, that were made by different users, which is extremely useful for web queries. Applying to similar data \cite {Sun:2009:PAB:1631272.1631351}, \cite {Yeh:2010:PPR:1873951.1873963} focused on exploring a personalized photo assessment. All those works focus on finalized images while our goal is to provide user a set which might be retouched and fixed. 

Smart clustering and event classification were explored by \cite{1223566}, \cite{4444209} while we chose the best image from each scene. 

\cite{springerlink:10.1007/978-3-642-10543-2_23} described a promissing method of automatically rating and ranking images based on image content and time-metadata, but they [reword it!!!] didn't provided any datasets and their user study showed that their results are better than random. Ha-ha, we are better!!!
...
Using the 48 image data set from Ke, Tang, and Jing\cite{1640788}, and Amazon Turk ratings as a ground truth, we achieve an accuracy of 80\%, and the authors achieve an accuracy of 45\%. We assume "accuracy" is a rating that is within one standard deviation of the distribution of users ratings per image.

Similarly, with Luo and Tang\cite{springerlink:10.1007/978-3-540-88690-7_29}, we achieve an accuracy of XX\%, and the authors an accuracy of XX\%, showing we are comparable.
