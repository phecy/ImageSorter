...
\(W_i\) - weight of module \(i\), \(n\) - number of modules, \(Q_i\) is  module i rating for the image, and \(t\) is a leniency threshold to balance each module's output, empirically chosen to be 4. Weights are assigned as follows: \(W_{exposure}=30\%\)
\(W_{blur}=60\%\)
\(W_{color}=10\%\)
...
Our code and data set is available at http://www.artoonie.com/imagesorter
