...
\maketitle
\begin{abstract}
In this paper, we propose a method for selecting the most important images from a set of user photographs. We filter out images which are technically flawed to provide the photographer with a high-quality representative subset of the imported images. Taking into account the redundancy of near-duplicates, we provide the user with an ordering of the best and most representative images. The filtered set chosen by our algorithm matches the user's choices with high accuracy (XX\%).
\end{abstract}
