...
\label{llfeat}
We use an ensemble of Support Vector Regression Machines (SVRs) to learn low-level features. 
UUU: the title says "low level" the whole section talks about high level,
but maybe it is ok, because the equations show the relationship,
but just from reading it is very confusing.
Not sure how to fix it... if u can, add a transition
sentence here, if not - ignore the comment.

Each SVR is trained to learn a single high-level feature (e.g. the last three columns of Figure \ref{fig:turktable}). The number of SVRs is equal to the number of high-level feature ratings provided by Turkers, as shown in Section \ref{turkdata}.
...
Where w is a vector of low level features weights, x is a vector of low level features,
b is a UUU.
...
<<<<<<< HEAD
Our application of the framework is intended to find low quality images. Previously, out of 74 low-quality photographs (which we collected from Turk users), we gave 66 a low rating (less than .5). After training our algorithm, we gave 61 a low rating- a slight decrease, but with a large improvement in speed.
=======
...
>>>>>>> 9e9eade125ae23dfc373281d035ba9e9c6289f08
...
UUU: 4.75 (out of 9) is the highest score we gave?
Our program is quite picky :)
